...
class _SavedEventsPageState extends State<SavedEventsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _openMapForEvent(Event event) async {
    if (event.address.isEmpty) {
      _showToast('No location available for this event', isError: true);
      return;
    }
    
    try {
      final success = await MapsLauncher.launchQuery(event.address);
      if (!success && mounted) {
        _showMapError();
      }
    } catch (e) {
      Logger.e('Error launching maps: $e', tag: 'SavedEvents_page');
      if (mounted) {
        _showMapError();
      }
    }
  ...
  Widget _buildStyledEventCard(Event event) {
    // Get category colors
    final categoryColor = eventCategoryColors[event.category.toUpperCase()] ?? app_colors.AppColors.primary;
    
    // Create a custom animation for removing events
    return material.Material(
      // Add a key for more efficient widget recycling
      key: ValueKey<String>(event.id),
      color: Colors.transparent,
      child: AnimatedOpacity(
        duration: const Duration(milliseconds: 300),
        opacity: _eventBeingRemoved[event.id] == true ? 0.0 : 1.0,
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            transform: Matrix4.identity()
              ..translate(
                _eventBeingRemoved[event.id] == true ? 100.0 : 0.0, 
                0.0, 
                0.0,
              ),
            ...
            decoration: BoxDecoration(
              color: app_colors.AppColors.backgroundDark.withValues(alpha: 0.7),
              ...
              children: [
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Top row with category and badges
                      Row(
                      ...
                      Row(
                        children: [
                          const Icon(
                            ...
                            color: app_colors.AppColors.backgroundLight,
                            size: 14,
                          ),
                      ...
                      // Date section - Enhanced to be more prominent
                            ...
                            Expanded(
                              child: Text(
                                ...
                                style: const TextStyle(
                                  ...
                                  decorationColor: app_colors.AppColors.backgroundLight,
                                ),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
                      ...
                      Row(
                        ...
                        children: [
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text(
                                    '${event.attendees.length}/${event.slots} slots',
                                    style: const TextStyle(
                                      color: app_colors.AppColors.backgroundLight,
                                      fontSize: 14,
                                      fontWeight: FontWeight.w500,
                              ...
                              if (event.attendees.isNotEmpty)
                                SizedBox(
                                  ...
                                  child: Row(
                                    children: [
                                      // Show up to 5 attendee avatars
                                      ...List.generate(
                                        event.attendees.length > 5 ? 5 : event.attendees.length,
                                        (index) => Padding(
                                          padding: EdgeInsets.only(right: index < 4 ? 4 : 0),
                                          child: _buildAttendeeAvatar(event.attendees[index]),
                                        ),
                                      ),
                          ...
                          Row(
                            children: [
                              // Unsave button with animated press effect
                              PlayAnimationBuilder<double>(
                                ...
                                builder: (context, value, child) {
                                  return GestureDetector(
                                    onTap: () async {
                                      await _removeFromSaved(event.id);
                                    },
                                    child: Transform.scale(
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: app_colors.AppColors.primary.withValues(alpha: 0.2),
                                          borderRadius: BorderRadius.circular(12),
                                          border: Border.all(
  ...
  Widget _buildCategoryBadge(String category) {
    final categoryColor = eventCategoryColors[category.toUpperCase()] ?? app_colors.AppColors.primary;
  ...
  Widget _buildDateSection(Event event) {
    ...
    final isTomorrow = eventDate.year == now.year && 
                       eventDate.month == now.month && 
                       eventDate.day == now.day + 1;
    
    // Display relative date (Today/Tomorrow) or full date
    String dateText;
    if (isToday) {
      dateText = 'Today';
    } else if (isTomorrow) {
      ...
      dateText = DateFormat('EEE, MMM d').format(eventDate); // e.g. "Fri, Sep 20"
    }
    
    // Format time with AM/PM
    final timeText = DateFormat('h:mm a').format(eventDate); // e.g. "7:30 PM"
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: app_colors.AppColors.primary.withValues(alpha: 0.15),
            ...
            Row(
              children: [
                Text(
                  timeText,
                  style: TextStyle(
                    color: app_colors.AppColors.backgroundLight.withValues(alpha: 0.8),
                    fontSize: 14,
                  ),
                ),
                if (event.timeLimit > 0) ...[
                  Text(
                    ' • ${event.timeLimit} min',
                    style: TextStyle(
                      color: app_colors.AppColors.backgroundLight.withValues(alpha: 0.6),
                      fontSize: 13,
                    ),
                  ),
                ],
  ...
  Widget _buildAttendeeAvatar(String attendeeId) {
    ...
    final userProfile = _userProfiles[attendeeId];
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        border: Border.all(
          color: app_colors.AppColors.backgroundLight.withValues(alpha: 0.2),
          width: 1,
        ),
        boxShadow: [
          BoxShadow(
      ...
      child: ClipRRect(
        ...
        child: userProfile != null && userProfile.photoUrl.isNotEmpty 
          ? CachedNetworkImage(
              imageUrl: userProfile.photoUrl,
              fit: BoxFit.cover,
              placeholder: (context, url) => _buildDefaultAvatar(attendeeId),
              errorWidget: (context, url, error) => _buildDefaultAvatar(attendeeId),
            )
          : _buildDefaultAvatar(attendeeId),
      ),
    );
  }
  
  Widget _buildDefaultAvatar(String attendeeId) {
    // Generate a consistent color based on the attendee ID
    final colorValue = attendeeId.hashCode % 0xFFFFFF;
    final avatarColor = Color(0xFF000000 + colorValue);
    ...
    return Container(
      ...
      child: Center(
        child: Text(
          attendeeId.isNotEmpty ? attendeeId[0].toUpperCase() : '?',
          style: const TextStyle(
            color: Colors.white,
            fontSize: 12,
            fontWeight: FontWeight.bold,
