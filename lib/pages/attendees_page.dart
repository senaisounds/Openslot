...
class _AttendeesPageState extends State<AttendeesPage> {
  ...
  Widget build(BuildContext context) {
    final filteredAttendees = userData.entries
        .where((entry) =>
            entry.value['username']!.toLowerCase().contains(searchQuery.toLowerCase()))
        .map((entry) => entry.key)
        .toList();
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final username = userData[performerId]?['username'] ?? performerId;
                  final photoUrl = userData[performerId]?['photoUrl'] ?? placeholderImage;
                                ...
                                CupertinoPageRoute(
                                  builder: (context) => CupertinoPageScaffold(
                                    ...
                                    child: ProfilePage(
                                      debug: widget.debug,
                                      user: widget.user,
                                      authAction: (loggedIn) => widget.authAction(context, loggedIn, () {}),
                                      viewUser: performerId,
                                    ),
                                  ),
                                ),
                              ),
